  ...
  let server = createServer({
    ...
    routes() {
      ...
      this.post("/account/login", (schema, request) => {
        ...
        if ((username === "sajjad", password === "123")) {
          let data = {
            username: "sajjad",
            token: "2342343",
            refreshToken: "t43543543545",
          };
          return data;
        } else {
          return new Response(
      ...
      this.post("/users", (schema, req) => {
        let res = JSON.parse(JSON.parse(req.requestBody).body);
        return schema.users.create(res);
      });
      ...
      //   let post = schema.blogPosts.find(user)
      //   return post.destroy()
      // })
    },
  });
